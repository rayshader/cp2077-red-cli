...
/// Manually implement [addDirectorySync], see https://github.com/brendan-duncan/archive/pull/393
/// for an official fix.
class ZipFileEncoderSync {
  ...
  ZipFileEncoderSync({this.password});
  ...
  Future<void> zipDirectory(Directory dir,
    ...
    await addDirectory(dir,
        includeDirName: false, level: level, followLinks: followLinks, onProgress: onProgress, filter: filter);
  ...
  void create(String zipPath, {int? level, DateTime? modified}) {
    createWithStream(OutputFileStream(zipPath), level: level, modified: modified);
  }
  ...
  void addFileSync(File file, [String? filename, int? level]) {
    ...
    final archiveFile = ArchiveFile.stream(filename ?? path.basename(file.path), fileStream);
  ...
  Future<void> addFile(File file, [String? filename, int? level]) async {
    ...
    archiveFile.lastModTime = (await file.lastModified()).millisecondsSinceEpoch ~/ 1000;
